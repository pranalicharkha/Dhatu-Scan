...
def analyze_visible_signs(
    ...
    # If model is available, use it as PRIMARY signal - minimal heuristic influence
    if model_probability is not None:
        # Model is the primary signal - use it directly with minimal adjustment
        # Only adjust if quality is very poor or signs strongly contradict
        if quality_score < 50:
            # Very poor quality - slightly reduce confidence
            wasting_probability = _clamp(model_probability * 0.9, 0.03, 0.97)
        elif sign_strength >= 2:
            # If clinical signs are detected (2+), force minimum high risk
            # This ensures OpenCV clinical signs override model output
            wasting_probability = max(_clamp(model_probability, 0.03, 0.97), 0.55)
        elif sign_strength >= 1 and model_probability < 0.3:
            # Some visible signs but model says very low - moderate boost
            wasting_probability = _clamp(model_probability + 0.25, 0.03, 0.97)
        else:
            # Use model probability directly - it's the primary signal
            wasting_probability = _clamp(model_probability, 0.03, 0.97)
    else:
        # No model - use heuristic as fallback with stronger sign influence
        heuristic_probability = _clamp(
            0.18 + sign_strength * 0.15 + quality_penalty * 0.20 + embedding_bonus + masking_penalty,
            0.03,
            0.97,
        )
        wasting_probability = heuristic_probability
    ...
    summary = (
        "Trained image model indicates elevated malnutrition risk (primary signal)."
